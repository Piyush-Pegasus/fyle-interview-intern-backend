...
class Assignment(db.Model):
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        assertions.assert_valid(grade in GradeEnum.__members__, 'API should allow only grades available in enum')
    ...
    def mark_grade_by_principal(cls, _id, grade, auth_principal: AuthPrincipal):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(assignment.state != AssignmentStateEnum.DRAFT, 'Only a submitted or already graded assignment can be graded or regraded')
        assertions.assert_valid(grade is not None, 'assignment with empty grade cannot be graded')
        assertions.assert_valid(grade in GradeEnum.__members__, 'API should allow only grades available in enum')
    ...
    #modified for test case 1 of teacher
    def get_assignments_by_teacher(cls,teacher_id):
        return cls.query.filter(cls.teacher_id==teacher_id).filter(cls.state!=AssignmentStateEnum.DRAFT).all()
